#include "main.h" // 替换为HAL库的主头文件，它会包含stm32fxxx_hal.h
#include "OLED_Font.h"

// 定义OLED的SCL和SDA引脚，这里假设你仍然使用GPIOB的8和9引脚
// 请根据你的实际硬件连接和STM32CubeMX配置进行修改
...
/*引脚配置*/
// 替换 GPIO_WriteBit 为 HAL_GPIO_WritePin
...
{
    GPIO_InitTypeDef GPIO_InitStruct = {0}; // HAL库的GPIO初始化结构体

    // 启用GPIOB时钟，HAL库通常在CubeMX生成的主文件中完成，
    // 但为确保独立性，这里也可以再次使能，或者依赖CubeMX的配置
    __HAL_RCC_GPIOD_CLK_ENABLE(); // 如果在main.c中未使能，可以在这里添加
